...
def generate_launch_description():
    ...
    robot_file = join(get_package_share_directory("krytn"), "robot_description","mobile_arm.urdf.xacro")
    ...
    start_controllers  = Node(
                ...
                executable="spawner",
                arguments=['joint_state_broadcaster', 'diff_drive_base_controller', 'arm_controller'],
                output="screen",
            )
    ...
    static_pub = Node(package="tf2_ros", 
                      ...
                      arguments=["0","0","0","0","0","0", "lidar_2d_link", "mobile_arm/base_footprint/lidar_2d_v1", ])
    
    static_pub2 = Node(package="tf2_ros", 
                      executable="static_transform_publisher",
                      arguments=["0","0","0","0","0","0",  "realsense_link", "mobile_arm/base_footprint/realsense_d435"])

    move_group = IncludeLaunchDescription(join(get_package_share_directory("mobile_robot_moveit"), "launch", "move_group.launch.py"))
    rviz = IncludeLaunchDescription(join(get_package_share_directory("mobile_robot_moveit"), "launch", "moveit_rviz.launch.py"))
    ...
    return LaunchDescription([gazebo_sim, bridge, robot, twist_stamper,
                              ...
                              move_group, rviz, mg_sim_time, rviz_sim_time ])